using ForBlog.Monster;
using ForBlog.Player;
using System;
...
namespace ForBlog.Game
{
    ...
    public class Game
    {
        ...
        {
            int ranMob = random.Next((int)Monster.Mob.FARMER, (int)Monster.Mob.WOLF + 1);
        ...
        private void Battle(Player.Player player, Monster.Monster monster)
        {
            while (player.GetCurrentHp() > 0)
            {
                ...
                if (player.GetConDmg()) player.IsContinuousDmg(monster);
                WriteLine("\n어떻게 공격할까?");
                player.ShowSkill();
        ...
        private void BossBattle(Player.Player player, ref Chapter chapter, ref int ranSkill)
        {
            ...
            if (chapter == Chapter.CHAPTER1)
            {
                ...
                while (player.GetCurrentHp() > 0 && boss.GetCurrentHp() > 0)
                {
                    ...
                    if (player.GetConDmg()) player.IsContinuousDmg(boss);
